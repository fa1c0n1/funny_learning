...
#define INSTRUCT_LENGTH 64 // 一条指令的最大长度
...
typedef struct  {
   Int32 Category;		// 指令集
   Int32 Opcode;		// 操作码(enum INSTRUCTION_TYPE)
   char Mnemonic[16];   // 操作码的助记符
   Int32 BranchType;	// 分支类型(enum BRANCH_TYPE)
   EFLStruct Flags;		// 状态寄存器
   UInt64 AddrValue;	// 如果当前指令是跳转指令,且跳转地址可计算,则保存跳转地址
   Int64 Immediat;		// 如果当前指令有立即数,则存储立即数
...
typedef struct _Disasm {
   UIntPtr EIP;							// [输入] 保存opcode的缓冲区首地址
   UInt64 VirtualAddr;					// [输入] 指令所在地址
   UInt32 SecurityBlock;				// 
   char CompleteInstr[INSTRUCT_LENGTH]; // [输出] 反汇编出来的汇编指令
   UInt32 Archi;						// [输入] 汇编指令的平台,X86(0),X64(64)
   UInt64 Options;						// 
   INSTRTYPE Instruction;				// [输出] 指令信息(opcode)
   ARGTYPE Argument1;					// [输出] 指令的第1个操作数
   ARGTYPE Argument2;					// [输出] 指令的第2个操作数
   ARGTYPE Argument3;					// [输出] 指令的第3个操作数
   PREFIXINFO Prefix;					// [输出] 指令前缀
   UInt32 Reserved_[40];				// 保留
