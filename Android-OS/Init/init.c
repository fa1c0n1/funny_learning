...
int main(int argc, char **argv)
{
    ...
    //创建一些文件夹, 并挂载设备, 这些是和Linux相关的, 不拟作过多讨论 
    mkdir("/dev", 0755);
    mkdir("/proc", 0755);
    mkdir("/sys", 0755);
    ...
    open_devnull_stdio();
    /*
        设置init的日志输出设备为 /dev/__kmsg__ , 不过该文件打开后, 会立即被unlink.
        这样, 其他进程就无法打开这个文件读取日志信息了.
    */
    log_init();
    
    INFO("reading config file\n");
    //上面涉及很多和Linux系统相关的知识, 但不影响我们分析
    //解析init.rc配置文件.
    ...
    //下面这个函数通过读取 /proc/cpuinfo得到机器的 Hardware名
    get_hardware_name();
    snprintf(tmp, sizeof(tmp), "/init.%s.rc", hardware);
    //解析这个和机器相关的配置文件,比如G7手机对应文件为init.bravo.rc
    parse_config_file(tmp);

    /*
        解析完上述两个配置文件后, 会得到一系列的Action(动作), 下面两句代码将执行
        那early-init阶段的Action. init将动作执行的时间划分为四个阶段: early-init、
        init、early-boot、boot.由于有些动作必须在其他动作完成后才能执行, 所以就有
        了先后之分. 哪些动作属于哪个阶段由配置文件决定. 后面会介绍配置文件的相关
        知识.
     */
    ...
    /*
        初始化 /dev/keychord设备, 这和调试有关. 本章节不讨论它的用法.
        有兴趣自己研究, 内容比较简单
     */
    ...
    /*
        INIT_IMAGE_FILE定义为"/initlogo.rle", 下面这个函数将加载这个文件作为系统
        的开机画面, 注意, 它不是开机动画控制程序bootanimation加载的开机动画文件.
    */
    if( load_565rle_image(INIT_IMAGE_FILE) ) {
    /*
        如果加载initlogo.rle文件失败(可能是没有这个文件), 则会打开/dev/tty0设备,并
    ...
    */
    ...
    //init关注来自四个方面的事件
    ufds[0].fd = device_fd; //[0]:device_fd用于监听来自内核的UEvent事件
    ufds[0].events = POLLIN;
    ufds[1].fd = property_set_fd; //[1]:property_set_fd用于监听来自属性服务器的事件
    ufds[1].events = POLLIN;
    ufds[2].fd = signal_recv_fd; //[2]:由socketpair创建,它的事件来自另外一个socket
    ufds[2].events = POLLIN;
    fd_count = 3;

    if (keychord_fd > 0) {
        //[3]:如果keychord设备初始化成功,则init也会关注来自这个设备的事件.
    ...
    //与BootChart相关,这里不做讨论
    /*
        BootChart是一个小工具,它能对系统的性能进行分析, 并生成系统启动过程的图表,
    ...
    /*
        从上面的代码可知, init的工作任务还是很重的.
        不过从本章节要分析的两个知识点来看, 可将init的工作流程精简如下：
            1) 解析两个配置文件, 其中, 将分析对init.rc文件的解析.
            2) 执行各个阶段的动作, 创建Zygote的工作就是在其中的某个阶段完成的. 
            3) 调用property_init初始化属性相关的资源, 并且通过
                property_start_service启动属性服务
            4) init进入一个无限循环, 并且等待一些事件的发生.
                这里重点关注init如何处理来自socket和来自属性服务器相关的事情.

        PS: 精简工作流程, 是以后分析代码时常用的方法.
     */
