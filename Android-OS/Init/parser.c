...
struct {
    const char *name; //关键字的名
    int (*func)(int nargs, char **args); //对应关键字的处理函数
    unsigned char nargs; //参数个数,每个关键字的参数个数是固定的

    //关键字的属性,有三种属性, COMMAND、OPTION和SECTION.
    //  其中COMMAND有对应的处理函数
    unsigned char flags;
} keyword_info[KEYWORD_COUNT] = {
    [ K_UNKNOWN ] = { "unknown", 0, 0, 0 },
//第二次包含 keywords.h, 由于已经重新定义了KEYWORD宏,所以以前那些作为枚举值的关键字
...
int parse_config_file(const char *fn)
{
    ...
    parse_config(fn, data); //调用parse_config对内容做真正的解析
    DUMP();
    return 0;
}
